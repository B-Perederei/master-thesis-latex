...
# Masker - just apply mask on weights (turn on turn off neurons)
# None_LSQ - stub, does nothing
# Full_part - implements QAT via STE and LSQ algorithms
# Masker_part - combines STE/LSQ with mask, first applying mask and after that applying QAT. Does backward path without mask
# Masker_part_fixed - similar to masker part, but uses masks for turning off pruned neurons during backward path
# LsqActivationFun and LsqActivation - implements Lsq for activations
# MaskConv2d - Conv2d with implemented QAT + pruning
# MaskLinear - Linear with QAT + pruning