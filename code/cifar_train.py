...
class KDLoss(nn.Module):
    def __init__(self, temperature=3.0, alpha=0.5, reduction='mean'):
        super().__init__()
...
class DKDLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, labels):
        return self.dkd_loss(student_logits, teacher_logits, labels, self.alpha, self.beta, self.temperature)

    # Taken from https://github.com/megvii-research/mdistiller
...
def main(args):
    ...
    if args.method == 'distill':
        if args.n_bits_teacher == 32 and args.n_bits_teacher == 32:
            teacher_model, _ = models.__dict__[args.arch](data=args.dataset, num_layers=args.layers,
                                                        width_mult=args.width_mult,
                                                        depth_mult=args.depth_mult,
                                                        model_mult=args.model_mult)
        else:
            pruner = pruning.__dict__[args.pruner]
            teacher_model, _ = pruning.models.__dict__[args.arch](data=args.dataset, num_layers=args.layers_teacher,
                                                    width_mult=args.width_mult,
                                                    depth_mult=args.depth_mult,
                                                    model_mult=args.model_mult,
                                                    mnn=pruner.mnn,
                                                    n_bit = args.n_bits_teacher,
                                                    acti_n_bits=args.acti_n_bits_teacher)
    ...
    if args.method == 'distill':
        if args.kd_method == 'dkd':
            criterion = DKDLoss(temperature=args.t, alpha=args.alpha, beta=args.beta)
        else:
            criterion = KDLoss(temperature=args.t, alpha=args.alpha)
    ...
    if args.method == 'distill':
        ...
        criterion = criterion.cuda()
        teacher_model = nn.DataParallel(teacher_model, device_ids=args.gpuids,
                                output_device=args.gpuids[0])
        cudnn.benchmark = True
    ...
    if args.method == 'distill':
        # strict = False if model is pruned
        load_model(teacher_model, args.teacher_path, main_gpu=args.gpuids[0], use_cuda=args.cuda, strict=False)
        print('==> Loaded Teacher \'{}\''.format(args.teacher_path))

        teacher_model.eval()
        temp = args.run_type
        temp1 = args.method
        args.run_type = 'evaluate'
        args.method = 'teacher'
        validate(args, val_loader, 0, teacher_model, nn.CrossEntropyLoss().cuda(), teacher_model=None)
        args.run_type = temp
        args.method = temp1
    ...
    if args.run_type == 'train':
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            acc1_train = train(args, train_loader,
                ...
                criterion=criterion, optimizer=optimizer,DS=DS, teacher_model=teacher_model)
            ...
            acc1_valid = validate(args, val_loader,
                epoch=epoch, model=model, criterion=criterion, teacher_model=teacher_model)
            elapsed_time = time.time() - start_time
            ...
            if epoch > 15:
                ...
                if is_best:
                    ...
                    # best_model_path = f"checkpoint/{arch_name}/{args.dataset}/{args.save}"
                    # wandb.save(best_model_path, base_path="checkpoint")
            ...
            epoch_metrics = {
                "epoch": epoch,
                "train/epoch_acc": acc1_train,
                "val/epoch_acc": acc1_valid,
                "train/learning_rate": optimizer.param_groups[0]["lr"]
            }
            if args.prune:
                epoch_metrics["pruning/sparsity"] = sparsity
        ...
        wandb.log({
            "time/avg_train_time_sec": avg_train_time,
            "time/avg_valid_time_sec": avg_valid_time,
            "time/total_train_time_sec": train_time,     
            "time/total_valid_time_sec": validate_time,
            "time/total_time_sec": total_train_time      
        })
        
        # save_model('FINAL_'+arch_name, args.dataset, state, args.save)
        final_model_path = f"checkpoint/{arch_name}/{args.dataset}/{args.save}"
        wandb.save(final_model_path, base_path="checkpoint")
        wandb.save("kd_study.db")
        ...
        save_eval([ckpt_name, acc1])

        wandb.log({"eval/final_acc": acc1})

        if args.prune:
            _,_,sparsity = pruning.cal_sparsity(model)
            print('Sparsity : {}'.format(sparsity))
            wandb.log({"eval/final_sparsity": sparsity})
        return acc1
    else:
        assert False, r'Unkown --run-type! It should be \{train, evaluate\}.'

def train(args, train_loader, epoch, model, criterion, optimizer,DS,teacher_model, **kwargs):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # for pruning
        if args.prune:
            if args.method == 'distill' or args.method == 'ours_freeze':
        ...
        if args.method == 'distill':
            if (args.kd_phase == 'full' or 
                (args.kd_phase == 'afterDynamic' and DS == 1) or 
                (args.kd_phase == 'afterSecondDecay' and DS == 1)): # DS == 1: # Enable KD only during static update
                with torch.no_grad():
                    if args.n_bits_teacher == 32 and args.n_bits_teacher == 32:
                        teacher_output = teacher_model(input) 
                    else:
                        teacher_output = teacher_model(input, 1, args.n_bits_teacher, args.acti_n_bits_teacher, args.acti_quan) 
                loss = criterion(output, teacher_output, target)
            else:
        ...
        if i % 20 == 0:
            ...
            wandb.log({
                ...
                "train/step_acc": top1.avg,
            })

    print("acc : ",100. * correct / len(train_loader.dataset))
    return 100. * correct / len(train_loader.dataset)

def validate(args, val_loader, epoch, model, criterion, teacher_model=None):
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if args.method == 'teacher':
                if args.n_bits_teacher == 32 and args.n_bits_teacher == 32:
                    ...
                    output = model(input, 1, args.n_bits_teacher, args.acti_n_bits_teacher, args.acti_quan)
                    # teacher_model(input, 1, args.n_bits_teacher, args.acti_n_bits_teacher, args.acti_quan)
            elif args.method == 'full_model':
                output = model(input)
            elif args.run_type == 'evaluate':
                output = model(input, 1, args.n_bits, args.acti_n_bits, args.acti_quan) # PARAMETER INFLUENCE IT 
            else:
                if (epoch + 1) <= 10 : 
                    output = model(input, 3, args.n_bits, args.acti_n_bits, 0)
                else:
            ...
            if args.method == 'distill':
                # loss = criterion(output, teacher_model(input, 1, args.n_bits_teacher, args.acti_n_bits_teacher, args.acti_quan), target)
...
if __name__ == '__main__':
    ...
    args = hyperparam()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.cu_num
    os.environ["WANDB_MODE"] = "online" # "online" "disabled"
    wandb.init(project="RAI Research", config=args)
    wandb.run.name = f"{args.arch}-{args.layers}_{args.dataset}_{args.method}_int{args.n_bits}_sparsity{args.prune_rate*100}" # OPTUNA_TRIAL{args.trial_num}_
    main(args)
    elapsed_time = time.time() - start_time
